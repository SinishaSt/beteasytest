...
/*
	Plugin name: Bet Easy Test Plugin
	Plugin URI: https://beateasy.com.au
	Description: Bet Easy test plugin.
	Version: 1.0.0
	Author: Sinisa Stankovic
	Author URI: http://accesspressthemes.com
	Text Domain: beateasy-plugin
*/
...
class BetEasy {
    ...
    function get_race() {
        ...
        if($data){
            $i = 1;
             do {
                foreach ($races as $race ) {
                    ...
                    $race_venue = $race['Venue']['Venue'];
                    $race_type =  $race['EventTypeDesc'];
                    $race_time =  $race['AdvertisedStartTime'];;
                    $date= date(' d/m/Y', strtotime($race_time));
                    $time = date(' H:i', strtotime($race_time));
                    ...
                    echo '<div data-type="'. strtolower($race_type) .'" class="list-item">';
                    echo '<div class="list-item-type">'. $list_icon .'</div>';
                    echo '<div class="list-item-venue">'. $race_venue .'<br><span>'. $race_name .'</span></div>';
                    echo '<div class="list-item-time">'. $date .'</br>'. $time .' h</div>';
                    echo ' </div>'; 
                    $i++;
                }
            ...
            return '<div class="be-error"> Sorry, there is no data! </div>';
        }
...
if( !class_exists( 'BeatEasy' ) ) {
   // Create new instance
   $beteasy = new BetEasy();
   //Call all scripts style/css
   $beteasy->register_front_end_scripts();
 
}

//add shortcode

add_shortcode( 'beteasy', array( $beteasy, 'get_race' ) );
